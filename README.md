![MasterHead](https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fcdn.weasyl.com%2F~fluffkevlar%2Fsubmissions%2F30165%2Fefb64790c6059bf9f32f9922bdfd36fad18bdd135aff5f67e99a7f0f29749042%2Ffluffkevlar-starfield-gif.gif&f=1&nofb=1&ipt=a9de411664d88a83ec9d8c5687d042bcf3bea032065b4f12ecbb4086d9a2b66f&ipo=images)

<table width="100%">
  <tr>
<td width="30%" valign="top">
  <h3>📌 The best Projects</h3>
  <ul>
    <li><a href="https://github.com/DaFi-1/tasknvim">tasknvim</a> - ⭐14</li>
  </ul>
  <h3>📁 Projects Overview</h3>
<ul>
  <li><a href="https://github.com/DaFi-1/DaFi-1/blob/main/DaFi.md">Ebooks</a> <span>(2 projects)</span></li>
  <li><a href="#">DevOps</a> <span>(0 projects)</span></li>
  <li><a href="#">Backend</a> <span>(1 projects)</span></li>
  <li><a href="#">FrontEnd</a> <span>(0 projects)</span></li>
  <li><a href="#">Deep Learning</a> <span>(1 projects)</span></li>
  <li><a>ALL</a> <span>(4 projects)</span></li>
</ul>
<h3>📁 Certifications</h3>
<ul>
  <li><a href="#">DevOps</a> <b>(0)</b> • ⏱ 0h</li>
  <li><a href="#">Backend</a> <b>(0)</b> • ⏱ 0h</li>
  <li><a href="#">FrontEnd</a> <b>(0)</b> • ⏱ 0h</li>
  <li><a href="#">Deep Learning</a> <b>(0)</b> • ⏱ 0h</li>
  <li><a>ALL</a> <b>(0)</b> • ⏱ 0h</li>
</ul>
</td>
    <td width="70%">
      <img src="https://ghstats.dev/api/card?username=DaFi-1&theme=transparent"             width="100%" />
      <img src="https://ghstats.dev/api/langs?username=DaFi-1&theme=transparent&layout=bar" width="100%" />
    </td>
  </tr>
</table>






<h3 id="project-overview">📁 Project Overview</h3>
<div style="margin-left: 1.5em;">

<details id="ebooks">
<summary><b>Ebooks (6)</b></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<ul style="margin: 0; padding-left: 1.2em; font-size: 13px; line-height: 1.4;">
  <li><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a> ✅</li>
  <li><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a> ❌</li>
  <li><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a> ❌</li>
  <li><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a> ❌</li>
  <li><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a> ❌</li>
  <li><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a> ❌</li>
</ul>
</details>

<details id="devops">
<summary><b>DevOps (4)</b></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<ul style="margin: 0; padding-left: 1.2em; font-size: 13px; line-height: 1.4;">
  <li><a href="#">Docker setup</a> ❌</li>
  <li><a href="#">CI/CD pipeline</a> ❌</li>
  <li><a href="#">Kubernetes lab</a> ❌</li>
  <li><a href="#">Monitoring system</a> ❌</li>
</ul>
</details>

<details id="backend">
<summary><b>Backend (6)</b></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<ul style="margin: 0; padding-left: 1.2em; font-size: 13px; line-height: 1.4;">
  <li><a href="#">API REST</a> ❌</li>
  <li><a href="#">Auth system</a> ❌</li>
  <li><a href="#">Microservices</a> ❌</li>
  <li><a href="#">Database project</a> ❌</li>
  <li><a href="#">Logging system</a> ❌</li>
  <li><a href="#">Queue system</a> ❌</li>
</ul>
</details>

<details id="frontend">
<summary><b>FrontEnd (3)</b></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<ul style="margin: 0; padding-left: 1.2em; font-size: 13px; line-height: 1.4;">
  <li><a href="#">Portfolio site</a> ❌</li>
  <li><a href="#">Dashboard UI</a> ❌</li>
  <li><a href="#">Landing page</a> ❌</li>
</ul>
</details>

<details id="deeplearning">
<summary><b>Deep Learning (5)</b></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<ul style="margin: 0; padding-left: 1.2em; font-size: 13px; line-height: 1.4;">
  <li><a href="#">CNN model</a> ❌</li>
  <li><a href="#">NLP project</a> ❌</li>
  <li><a href="#">Image classifier</a> ❌</li>
  <li><a href="#">LLM experiment</a> ❌</li>
  <li><a href="#">Transformer model</a> ❌</li>
</ul>
</details>

<details id="other">
<summary><b>Other (1)</b></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<ul style="margin: 0; padding-left: 1.2em; font-size: 13px; line-height: 1.4;">
  <li><a href="#">CNN model</a> ❌</li>
</ul>
</details>

</div>

<hr>

<h3 id="certifications">📁 Certifications</h3>
<div style="margin-left: 1.5em;">

<details id="cert-devops">
<summary><b>DevOps (4) • 200h</b></summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
  <img src="https://img.shields.io/badge/Docker%20Course-Certificate-blue" alt="Docker Course" />
  <img src="https://img.shields.io/badge/Kubernetes%20Course-Certificate-blue" alt="Kubernetes Course" />
  <img src="https://img.shields.io/badge/CI%2FCD%20Course-Certificate-blue" alt="CI/CD Course" />
  <img src="https://img.shields.io/badge/Monitoring%20Course-Certificate-blue" alt="Monitoring Course" />
</div>
</details>

<details id="cert-backend">
<summary><b>Backend (0) • 0h</b></summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;"></div>
</details>

<details id="cert-frontend">
<summary><b>FrontEnd (0) • 0h</b></summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;"></div>
</details>

<details id="cert-deeplearning">
<summary><b>Deep Learning (5) • 300h</b></summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
  <img src="https://img.shields.io/badge/Neural%20Networks-Certificate-blue" alt="Neural Networks" />
  <img src="https://img.shields.io/badge/CNN-Certificate-blue" alt="CNN" />
  <img src="https://img.shields.io/badge/NLP-Certificate-blue" alt="NLP" />
  <img src="https://img.shields.io/badge/Transformers-Certificate-blue" alt="Transformers" />
  <img src="https://img.shields.io/badge/PyTorch-Certificate-blue" alt="PyTorch" />
</div>
</details>

</div>











<h3 id="project-overview">📁 Projects Overview</h3>
<div style="margin-left: 1.5em;">

<details id="ebooks">
<summary><a href="https://github.com/DaFi-1/DaFi-1/blob/main/DaFi.md">Ebooks</a> <span style="font-size:12px; color:#666;">(2 projects)</span></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<table width="100%" style="border-collapse: collapse; font-size: 13px; line-height: 1.1;">
<tr><th style="padding:2px 4px; width:36px;">#</th><th style="padding:2px 4px;">Ebook-Project</th><th style="padding:2px 4px; width:110px;"></th></tr>
<tr><td style="padding:2px 4px;">1</td><td style="padding:2px 4px;"><a href="https://github.com/DaFi-1/Ebook-TDD-Desenvolvimento-Guiado-por-Test.git">Ebook-TDD-Desenvolvimento-Guiado-por-Test</a></td><td style="padding:2px 4px;">✅</td></tr>
<tr><td style="padding:2px 4px;">2</td><td style="padding:2px 4px;"><a href="#">Project 2</a></td><td style="padding:2px 4px;">❌</td></tr>
</table>
</details>

<details id="devops">
<summary><a href="#">DevOps</a> <span style="font-size:12px; color:#666;">(0 projects)</span></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<table width="100%" style="border-collapse: collapse; font-size: 13px; line-height: 1.1;">
<tr><th style="padding:2px 4px; width:36px;">#</th><th style="padding:2px 4px;">DevOps-Project</th><th style="padding:2px 4px; width:110px;"></th></tr>
</table>
</details>

<details id="backend">
<summary><a href="#">Backend</a> <span style="font-size:12px; color:#666;">(1 project)</span></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<table width="100%" style="border-collapse: collapse; font-size: 13px; line-height: 1.1;">
<tr><th style="padding:2px 4px; width:36px;">#</th><th style="padding:2px 4px;">Backend-Project</th><th style="padding:2px 4px; width:110px;"></th></tr>
<tr><td style="padding:2px 4px;">1</td><td style="padding:2px 4px;"><a href="#">API REST</a></td><td style="padding:2px 4px;">❌</td></tr>
</table>
</details>

<details id="frontend">
<summary><a href="#">FrontEnd</a> <span style="font-size:12px; color:#666;">(0 projects)</span></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<table width="100%" style="border-collapse: collapse; font-size: 13px; line-height: 1.1;">
<tr><th style="padding:2px 4px; width:36px;">#</th><th style="padding:2px 4px;">FrontEnd-Project</th><th style="padding:2px 4px; width:110px;"></th></tr>
</table>
</details>

<details id="deeplearning">
<summary><a href="#">Deep Learning</a> <span style="font-size:12px; color:#666;">(1 project)</span></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">✅ Concluído | ❌ Em Desenvolvimento</p>
<table width="100%" style="border-collapse: collapse; font-size: 13px; line-height: 1.1;">
<tr><th style="padding:2px 4px; width:36px;">#</th><th style="padding:2px 4px;">DeepLearning-Project</th><th style="padding:2px 4px; width:110px;"></th></tr>
<tr><td style="padding:2px 4px;">1</td><td style="padding:2px 4px;"><a href="#">CNN model</a></td><td style="padding:2px 4px;">❌</td></tr>
</table>
</details>

<details id="all-projects">
<summary><a>ALL</a> <span style="font-size:12px; color:#666;">(4 projects)</span></summary>
<p style="font-size: 11px; color: #666; margin: 4px 0 8px 0;">Total projects listed above.</p>
</details>

</div>

<hr>

<h3 id="certifications">📁 Certifications</h3>
<div style="margin-left: 1.5em;">

<details id="cert-devops">
<summary><a href="#">DevOps</a> <b>(0)</b> • ⏱ 0h</summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
</div>
</details>

<details id="cert-backend">
<summary><a href="#">Backend</a> <b>(0)</b> • ⏱ 0h</summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
</div>
</details>

<details id="cert-frontend">
<summary><a href="#">FrontEnd</a> <b>(0)</b> • ⏱ 0h</summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
</div>
</details>

<details id="cert-deeplearning">
<summary><a href="#">Deep Learning</a> <b>(0)</b> • ⏱ 0h</summary>
<div style="margin-top: 8px; display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 8px;">
</div>
</details>








